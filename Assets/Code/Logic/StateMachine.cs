...
public class StateMachine : MonoBehaviour 
{
	...
	public bool		AddState(int _stateID, StateMode.DelStart _startDel, StateMode.DelUpdate _updateDel, StateMode.DelEnd _endDel, string stateName = "???" )
	{
		if (this.stateModes.ContainsKey(_stateID))
			return false;			//Mode already exists
	
		StateMode newStateMode = new StateMode(_stateID, _startDel, _updateDel ,_endDel, this.stateMachineName, stateName);
	...
	{
		if (this.stateModes.ContainsKey(this.activeState))
			return this.stateModes[this.activeState].stateName;
	...
	{
		return string.Format(">>>>>{0} {1} ID {2} time {3:0.0}s",this.stateMachineName, this.ActiveStateName(), this.activeState, this.ActiveStateTime());
	}
...
public class StateMode
{
	...
	public StateMode(int _stateID, DelStart _startState, DelUpdate _updateState, DelEnd _endState, string _ownerMachine = "????", string _stateName = "???")
	{
