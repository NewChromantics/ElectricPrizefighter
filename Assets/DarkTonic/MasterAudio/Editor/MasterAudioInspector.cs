...
public class MasterAudioInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        var newMasterVol = DTGUIHelper.DisplayVolumeField(_sounds._masterAudioVolume, DTGUIHelper.VolumeFieldType.None, topSliderWidth);
        ...
        if (plControllerInScene) {
            ...
            GUILayout.Space(5);
            var newPlaylistVol = DTGUIHelper.DisplayVolumeField(_sounds._masterPlaylistVolume, DTGUIHelper.VolumeFieldType.None, topSliderWidth);
            ...
            GUILayout.Space(5);
            var newCrossTime = EditorGUILayout.Slider(_sounds.crossFadeTime, 0f, 10f, topSliderWidth == MasterAudio.MixerWidthMode.Narrow ? GUILayout.Width(115) : GUILayout.Width(252));
        ...
        if (_sounds.areGroupsExpanded) {
            ...
            if (groups.Count == 0) {
                ...
                if (GUILayout.Button(new GUIContent("Max Grp. Volumes", "Reset all group volumes to full"), EditorStyles.toolbarButton, GUILayout.Width(104))) {
                    UndoHelper.RecordObjectsForUndo(groups.ToArray(), "Max Grp. Volumes");
                ...
                var newWidth = (MasterAudio.MixerWidthMode)EditorGUILayout.EnumPopup("", _sounds.mixerWidth, GUILayout.Width(60));
                if (newWidth != _sounds.mixerWidth) {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Mixer Width");
                ...
                for (var i = 0; i < pcs.Count; i++) {
                    ...
                    if (_sounds.mixerWidth != MasterAudio.MixerWidthMode.Narrow) {
                        ...
                        if (newSync != syncIndex) {
                            UndoHelper.RecordObjectPropertyForUndo(ref ctrlDirty, controller,
            ...
            if (_sounds.playlistsExpanded) {
                ...
                for (var i = 0; i < _sounds.musicPlaylists.Count; i++) {
                    ...
                    if (aList.isExpanded) {
                        ...
                        GUILayout.Space(10);
                        GUI.contentColor = DTGUIHelper.BrightButtonColor;
                        if (GUILayout.Button(new GUIContent(theButtonText), EditorStyles.toolbarButton, GUILayout.Width(70))) {
